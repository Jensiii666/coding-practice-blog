...
def register():
    ...
    if request.method == "POST":
        ...
        password = werkzeug.security.generate_password_hash(password=password, method="pbkdf2:sha256", salt_length=8)
...
def login():
    ...
    if request.method == "POST":
        ...
        password = request.form["password"]
        hash = user.password
        correct_pw = werkzeug.security.check_password_hash(pwhash=hash, password=password)
        if correct_pw:
            login_user(user)
            return redirect(url_for("get_all_posts"))
...
def show_post(post_id):
    ...
    if request.method == "POST":
        if not current_user.is_authenticated:
            flash("You have to log in to post a comment.")
            return redirect(url_for("login"))
